...
WebDKP_DkpAwardAll =		"WebDKP: $dkp dkp awarded to all players cuz: $reason.";

WebDKP_DkpAwardSome =		"WebDKP: $dkp dkp awarded to following players cuz: $reason.";

WebDKP_BidStart =			"WebDKP: Bid round has started for $item! $time " ..
							"To bid write !bid <value> in whisper. "..
							"(ex: !bid 50)";
...
function WebDKP_AnnounceBidEnd(item, name, dkp)
	...
	--local _,_, link = WebDKP_GetItemInfo(item);
	local tellLocation = WebDKP_GetTellLocation();
	local toSay = string.gsub(WebDKP_BidEnd, "$item", item);
